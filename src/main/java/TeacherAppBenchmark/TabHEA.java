...
public class TabHEA<K,V> extends MyHash<K,V>{
    ...
    public TabHEA() {
        ...
        actualSize = 0;
        calcMaxSize();
        hashVector = new Item[this.length];
    }

    public TabHEA(int length) {
        this.hashEngine = new HashDefault();
    ...
    public TabHEA(HashEngine hashEngine) {
    ...
    public TabHEA(int length, HashEngine hashEngine) {
        this.hashEngine = hashEngine;
        this.length = Prime.calcArraySize(length);
        actualSize = 0;
        calcMaxSize();
    ...
    public boolean insertItem(K key, V value) {
        ...
        try {
            ...
            if(posFind != -1)
            {
                ...
                if(firstNull != -1)
                {
                    hashVector[firstNull] = new Item<>(code,key,value);
                    actualSize++;
                    return true;
                }
                return false;
